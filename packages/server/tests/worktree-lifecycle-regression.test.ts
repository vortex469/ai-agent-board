...
function registerFakeProvider(
  ...
  execute: (
    ...
    onEvent: (event: { id: string; type: string; content: string; timestamp: number; metadata?: Record<string, unknown> }) => void,
  ) => Promise<{ status: 'complete' | 'failed'; error?: string; output?: string }>,
): void {
  const provider = {
    ...
    createSession: async ({ workingDirectory, onEvent }: { workingDirectory: string; onEvent?: (event: { id: string; type: string; content: string; timestamp: number; metadata?: Record<string, unknown> }) => void }) => ({
      execute: async () => {
        const result = await execute(workingDirectory, (event) => onEvent?.(event));
        if (result.output) {
...
test('successful startAgentForTask run auto-merges clean committed worktree and moves task to done', async () => {
  const f = fixture();
  const manager = new AgentManager();
  registerFakeProvider(manager, async (workingDirectory, onEvent) => {
    writeFileSync(path.join(workingDirectory, 'auto-merged.txt'), 'auto merged\n');
    onEvent({
      id: randomUUID(),
      type: 'test_result',
      content: 'verified auto-merged.txt was written',
      timestamp: Date.now(),
      metadata: { command: 'node --test worktree-lifecycle-regression.test.ts', state: 'succeeded' },
    });
    return {
      status: 'complete',
...
test('successful Python test runs with ignored pytest and ruff caches still auto-merge to done', async () => {
  ...
  registerFakeProvider(manager, async (workingDirectory, onEvent) => {
    ...
    onEvent({
      id: randomUUID(),
      type: 'test_result',
      content: 'pytest and ruff completed and left ignored cache artifacts only',
      timestamp: Date.now(),
      metadata: { command: 'python -m pytest && ruff check .', state: 'succeeded' },
    });
    return {
      status: 'complete',
...
test('startAgentForTask leaves completed task in review when auto-merge readiness is blocked', async () => {
  ...
  registerFakeProvider(manager, async (workingDirectory, onEvent) => {
    ...
    onEvent({
      id: randomUUID(),
      type: 'test_result',
      content: 'verified blocked.txt was written',
      timestamp: Date.now(),
      metadata: { command: 'node --test worktree-lifecycle-regression.test.ts', state: 'succeeded' },
    });
    return {
